...
const TrendingStore = () =>
{
  ...
  const {state, fetchTrendingProducts} = useProduct();
    ...
    <article className="mx-4 px-5 py-8 ring-1  ring-gray-200 rounded-2xl">
      <section className="w-full flex  justify-between items-center px-4 mb-5">
        ...
        <div className="flex gap-10 items-center-safe justify-center-safe ">
          <NavLink to={'products/trending'} className="flex gap-2 items-center-safe justify-center-safe ring-1 ring-[#DE57C4] py-2 px-2 rounded-full font-semibold text-[clamp(0.6rem,3vw,1.1rem)]">
            View All ({state.trending.total > 99 ? '+ 99': state.trending.total})
      ...
      <section className=" flex  justify-between items-center mb-5">
        <div className="w-full flex justify-between items-center-safe  " >
          ...
          <ScrollAction scrollRef={scrollRef} canScrollRight={canScrollRight} canScrollLeft={canScrollLeft}  totalProducts={state.trending.total} disabled_view={true}  />
        </div>
      </section>
      ...
      <section className="mt-2">
        <div className="w-full grid grid-flow-col gap-5 overflow-x-auto snap-x snap-mandatory no-scrollbar" ref={scrollRef}>
...

          {
            ...
            state.trending.total > 0 ?  
            state.trending.product.map(product =>  <ProductDetail key={product.product_id} name={product.name} price={product.price} discount={product.discount_price} image={product.productImage} slug={product.product_slug} />)
          :
            <>
              <ProductDetail />
              <ProductDetail />
