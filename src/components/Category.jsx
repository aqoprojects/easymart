  ...
  return (
    <section className='py-2 mx-auto px-4 mt-3 mb-12'>
      <div className='w-full justify-evenly md:justify-center-safe flex-wrap md:flex-nowrap  items-center flex md:overflow-x-auto snap-x snap-mandatory no-scrollbar  space-x-2 space-y-1.5'>
        {
          productCategories.length > 0 ? productCategories.map( category => <ProductCategory truncate={'truncate'} name={category.name} icon={category.Icon} />) : <div className="w-150 h-9 flex justify-center items-center-safe rounded-full bg-gray-200 animate-pulse"></div>
        }
